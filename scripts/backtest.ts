/**
 * Backtest: paper trading over RECORDED markets (brief §38). For paper
 * trading against live books see scripts/paper.ts (pnpm bot:paper).
 *
 *   pnpm backtest                    # cached Jev answers only
 *   pnpm backtest -- --jev           # call Jev for uncached states
 *   pnpm backtest -- --latency 400 --seed 7
 *
 * Output: data/backtest.sqlite and reports/backtest-summary.json. Never sends
 * anything anywhere.
 */
...
const outDb = openDatabase("data/backtest.sqlite");
...
console.log(`backtest: ${outcomes.length} resolved market(s), latency ${latencyMs} ms, seed ${seed}, jev ${call ? "cache-then-call" : "cache only"}\n`);
