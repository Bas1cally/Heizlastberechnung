...
while (!stopping) {
  ...
  const code = await new Promise<number>((resolve) => {
    child = spawn(process.execPath, ["node_modules/tsx/dist/cli.mjs", script, ...extraArgs, ...process.argv.slice(3)], { stdio: "inherit", env: { ...process.env, ...extraEnv, AUTO_RESTART: "1" } });
