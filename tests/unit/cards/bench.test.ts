...
describe("jev teams", () => {
  it("extra state reaches Jev; the guide is one text-model call; headlines summarise each run", async () => {
    ...
    const bodies: string[] = [];
    const rowsFor = (hands: string[]) => ({ rows: hands.map((hand) => ({ hand, vs_2: "H", vs_3: "H", vs_4: "S", vs_5: "S", vs_6: "S", vs_7: "H", vs_8: "H", vs_9: "H", vs_10: "H", vs_A: "H" })) });
    const all = ["hard 5-8", "hard 9", "hard 10", "hard 11", "hard 12", "hard 13", "hard 14", "hard 15", "hard 16", "hard 17+", "soft A2", "soft A3", "soft A4", "soft A5", "soft A6", "soft A7", "soft A8", "soft A9", "pair 22", "pair 33", "pair 44", "pair 55", "pair 66", "pair 77", "pair 88", "pair 99", "pair TT", "pair AA"];
    const fetch: FetchLike = async (_u, init) => { bodies.push(init?.body ?? ""); const content = bodies.length === 1 ? rowsFor(all.slice(0, 10)) : rowsFor(all); const b = { choices: [{ message: { content: JSON.stringify(content) } }], usage: { prompt_tokens: 100, completion_tokens: 200 } }; return { status: 200, ok: true, json: async () => b, text: async () => JSON.stringify(b), headers: { get: () => null } }; };
    const g = await makeGuide("blackjack", { apiKey: "k", model: "m", fetch });
    expect(bodies).toHaveLength(2);
    expect(bodies[1]).toContain("missed these rows");
    expect(g.guide.split("\n")).toHaveLength(29);
    expect(g.guide).toContain("hard 12: 2 H, 3 H, 4 S");
    expect(g.tokens).toBe(600);
    const short: FetchLike = async () => { const b = { choices: [{ message: { content: JSON.stringify({ guide: "x".repeat(60) }) } }], usage: {} }; return { status: 200, ok: true, json: async () => b, text: async () => JSON.stringify(b), headers: { get: () => null } }; };
    await expect(makeGuide("equity", { apiKey: "k", model: "m", fetch: short })).rejects.toThrow(/too short/);
