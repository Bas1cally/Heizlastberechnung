...
function render(s){
 ...
 if(mk){const total=(mk.closesAtMs-mk.openedAtMs)/1000;const rem=Math.max(0,(mk.closesAtMs-s.now)/1000);const done=1-rem/total;
  ...
  $('market').innerHTML=(live?'<div class="big num">'+Math.floor(rem/60)+':'+String(Math.floor(rem%60)).padStart(2,'0')+' <span class="muted" style="font-size:14px">verbleibend</span></div><div class="bar" style="margin:6px 0 12px"><i style="width:'+(done*100).toFixed(1)+'%"></i></div>':'<div class="muted" style="margin-bottom:10px">Warte auf das nächste 5-Minuten-Fenster.</div>')
   +(d?row('TWAP bei Start → jetzt',f(d.settlementStart,0)+' → '+f(d.settlementCurrent,0))+row('Veränderung (zählt)',(d.distanceBps>=0?'▲ +':'▼ ')+f(d.distanceBps,2)+' bps')+(d.spotPrice!=null?row('Spot vs TWAP',f(d.spotPrice,0)+' · '+(d.spotVsTwapBps>=0?'+':'')+f(d.spotVsTwapBps,2)+' bps'):'')+row('Up-Token (Kauf)',f(d.upAsk))+row('Down-Token (Kauf)',f(d.downAsk))+row('Paar zusammen',f(d.pairAskCost,4)+(d.pairEdge>0?' · Arbitrage!':''))+row('Bestand',d.inventory?(d.inventory.upShares+' Up · '+d.inventory.downShares+' Down'):'—'):'');
 } else {$('msub').textContent='—';$('market').innerHTML='<span class="muted">Noch kein Markt aufgezeichnet.</span>';}
 if(d){$('jsub').textContent='vor '+d.ageS+' s · Grund: '+({first:'erster Blick',quote:'Kurs bewegt',pair:'Paarkosten bewegt',settlement:'TWAP bewegt',spot:'Spot bewegt',time_bucket:'Zeitfenster gewechselt',inventory:'Bestand geändert',data_quality:'Datenqualität',heartbeat:'Routine-Check'}[d.materialReason]||d.materialReason||'—');
