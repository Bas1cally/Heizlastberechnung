...
export function collectState(db: Db, nowMs: number): Record<string, unknown> {
  ...
  return {
    ...
    feeds: { chainlinkAgeS: ticks?.last ? Number(((nowMs - ticks.last) / 1000).toFixed(1)) : null, bookAgeS: books?.last ? Number(((nowMs - books.last) / 1000).toFixed(1)) : null },
...
export function startDashboard(db: Db, port: number, log: (msg: string) => void): () => void {
  const repo = new DecisionRepository(db);
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && url === "/api/state") return json(res, 200, collectState(db, Date.now()));
