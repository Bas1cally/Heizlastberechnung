 ...
 * full except the high-volume ones (ticks, trades, books), which are windowed
 * (`sinceMs`) and, for books, thinned to roughly one snapshot per asset per 10 s. Decisions,
...
export function exportCompact(srcPath: string, outPath: string, sinceMs: number): { tables: Record<string, number> } {
  ...
  try {
    ...
    if (db.prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'trades'`).get()) {
      db.prepare(`CREATE TABLE out.trades AS SELECT * FROM main.trades WHERE received_at_ms >= ?`).run(sinceMs);
      tables["trades"] = (db.prepare(`SELECT COUNT(*) AS n FROM out.trades`).get() as { n: number }).n;
    }
