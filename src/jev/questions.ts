...
export const QUESTIONS = {
  action: choice(
    "Given the settlement state, market prices, time remaining, market dynamics and the existing inventory, what is the best immediate action? The economics that matter: a complete set (one UP plus one DOWN share) always merges back to exactly 1.00, so a pair bought for 1.00 or less costs nothing. Buying the trailing side (`tailAsk`, usually 0.01-0.02 late in the market) and later pairing it with the leading side at no more than `hedgePriceCap` is therefore a free option on a late reversal: if the lead reverses before the hedge, the tail pays 1.00; if not, the pair is merged at no cost. Late in the market the leader usually has no ask at all (`leaderAskDepth` 0): the hedge is then a bid resting at `hedgePriceCap` that fills only when holders of the leader sell into it, which is how the reference trader got nearly all of his hedges; `hedgeAvailable` says the hedge can be lifted at once instead. An unpaired tail whose hedge never fills expires worthless if the lead holds. `openOrders` counts our orders already resting or in flight: asking for a hedge again while one rests only duplicates it. `leadHeldRate` is the measured chance the lead holds. Two things this does not mean: early in the market both sides trade near 0.50, so buying either is a plain directional bet that only `leadHeldRate` and the movement evidence can justify, not a cheap option; and a set that costs 1.00 or more earns nothing at all. The trailing side is a tail worth holding as an option only when `tailAsk` is a few cents (about 0.05 or less), i.e. when the market already treats the outcome as decided.",
    {
